...
app.post('/api/messages', async (req, res) => {
    ...
    try {
        ...
        // что сами только что зашифровали и отправили в БД строкой выше.
        fullMessage.text = safeText;
            ...
            setTimeout(async () => {
                ...
                try {
                    const botResult = await pool.query(
                        ...
                        [chatId, roomId, botUserId, encryptText(randomResponse), 0, botTime, 'read']
                    );
                    ...
                    if (botMessage) {
                        botMessage.text = randomResponse;
...
app.post('/api/messages/file', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            ...
            [chatId, roomId, req.session.userId, encryptText(messageText), fileUrl, sanitizedFileName, fileType, messageType, 1, time, 'sent']
        );
        const messageId = result.rows[0].id;
...
app.get('/api/search', async (req, res) => {
    ...
    try {
        ...
        // Текст сообщений теперь хранится зашифрованным, поэтому ILIKE по
        // m.text на стороне БД больше не работает (шифротекст с одним и тем
        // же IV никогда не повторяется, даже для одинакового исходного
        // текста). Забираем кандидатов (последние сообщения искателя по
        // всем его чатам, с запасом по лимиту) и фильтруем уже после
        // расшифровки на стороне приложения.
        //
        ...
        const messages = candidates
            ...
            .filter(m => typeof m.text === 'string' && m.text.toLowerCase().includes(needle))
            .slice(0, 20);
